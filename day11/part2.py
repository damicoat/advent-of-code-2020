...
def get_num_occupied_around(grid, i, j):
  ...
  for idx1 in range(len(around)):
    for idx2 in range(len(around)):
      num = around[idx1]
      num2 = around[idx2]
      ...
      # find any seat on the slope
      search_for_seat = True
      while search_for_seat:
        search_for_seat = False

        if (0 <= i + num < len(grid)) and (0 <= j + num2 < len(grid[i + num])):
          if grid[i + num][j + num2] == '#':
            occupied += 1
          elif grid[i + num][j + num2] == 'L':
            break
          else:
            search_for_seat = True
            num += around[idx1]
